...
function SidebarMenuSkeleton({
  ...
  // Slight width variation so a column of skeleton rows doesn't look like a
  // perfect grid. `useId` gives us a deterministic-but-different string per
  // instance — we hash a couple of bytes off it to land in the [50, 90] range.
  const id = React.useId();
  const width = `${50 + ((id.charCodeAt(id.length - 1) ?? 0) % 40)}%`;
  return (
    <div
